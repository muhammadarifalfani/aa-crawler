...
plus one optional ``--output`` path (ADR-027), an optional ``--sink``
choice of concrete persistence sink (ADR-031, valid only with
``--output``), and an optional ``--interval``/``--max-runs`` pair
selecting scheduled crawl mode (ADR-028). It delegates execution to
:mod:`aa_crawler.cli.app`, :mod:`aa_crawler.cli.scheduler`, or
:mod:`aa_crawler.cli.batch`, returning the resulting process exit code.
...
#: Resolves --sink's string choice to a concrete sink class (ADR-031).
#: "file" preserves ADR-027's exact original default.
_SINK_FACTORIES: dict[str, SinkFactory] = {
    "file": FileCrawlResultSink,
    "sqlite": SqliteCrawlResultSink,
}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional file path to also save each crawl result to "
            "(ADR-027). Omitted by default: no file is written unless "
            "this is supplied."
        ),
    )
    parser.add_argument(
        "--sink",
        choices=sorted(_SINK_FACTORIES),
        default=None,
        help=(
            "Optional concrete persistence sink to use with --output "
            "(ADR-031). 'file' appends one JSON Lines record per result "
            "(ADR-027's original behavior; the default when --sink is "
            "omitted). 'sqlite' upserts by requested_url into a SQLite "
            "database instead, giving real idempotency for repeated "
            "re-crawls of the same URL (ADR-030). Only valid together "
            "with --output."
        ),
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    Returns:
        ...
        ``--urls-file`` and ``--interval`` were supplied. ``--sink``
        (ADR-031) selects which concrete sink class each of those
        receives as ``sink_factory`` when ``--output`` is supplied.
    """
    ...
    if args.sink is not None and args.output is None:
        parser.error("--sink requires --output")

    if (args.url is None) == (args.urls_file is None):
        parser.error("exactly one of url or --urls-file is required")

    sink_factory = _SINK_FACTORIES[args.sink or "file"]
    ...
    if args.urls_file is not None:
        ...
        if args.interval is None:
            return run_batch_crawl(urls, output=args.output, sink_factory=sink_factory)
    ...
    if args.interval is None:
        return run_crawl(args.url, output=args.output, sink_factory=sink_factory)
